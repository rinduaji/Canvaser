									...
									<div class="card card-xl-stretch mb-5 mb-xl-12">
										...
										<div class="card-header border-0 pt-5">
											<h3 class="card-title align-items-start flex-column">
												<span class="card-label fw-bolder fs-3 mb-1">Approve QCO</span>
						...
						<div class="stepper stepper-pills stepper-column d-flex flex-column flex-xl-row flex-row-fluid" id="kt_modal_create_app_stepper">
							...
							<div class="flex-row-fluid py-lg-5 px-lg-15">
								...
								<form class="form" novalidate="novalidate" id="kt_modal_create_app_form">
									...
									<input type="hidden" class="form-control form-control-lg form-control-solid" name="tgl_qco1" value="<?=date("Y-m-d h:i:s")?>" readonly/>
									<!--begin::Step 1-->
									<div class="current" data-kt-stepper-element="content">
										<div class="w-100">
											...
											<div class="fv-row mb-10">
												...
												<!--end::Label-->
												<!--begin::Input-->
												<textarea class="form-control form-control-lg" name="rec_qco" placeholder="" required></textarea>
												<!--end::Input-->
											</div>
											<div class="fv-row mb-10">
												<!--begin::Label-->
												<label class="d-flex align-items-center fs-5 fw-bold mb-2">
													<span class="required">Pilihan Tapping</span>
												</label>
												<!--end::Label-->
												<!--begin::Input-->
												<select class="form-control form-control-lg form-select" name="pilihan" placeholder="" required>
													<option value="">-- PILIH TAPPING --</option>
		...
		<script>
			...
			$(document).ready(function() {
				...
				table = $('#table').DataTable({ 
					...
					"ajax": {
						"url": "<?php echo site_url('approve/ajax_list')?>",
						"type": "POST"
					},
			...
			function tapping_data(id)
			{
				...
				$.ajax({
					url : "<?php echo site_url('approve/ajax_edit/')?>/" + id,
					type: "GET",
					dataType: "JSON",
			...
			function save()
			{
				...
				if(save_method == 'add') {
					url = "<?php echo site_url('approve/ajax_add')?>";
				} else {
					url = "<?php echo site_url('approve/ajax_update')?>";
				}
				...
				$.ajax({
					...
					success: function(data)
					{
						...
						if(data.status) //if success close modal and reload ajax table
						{
							...
							reload_table();
							swal("Success", "Data Berhasil Di Tapping!", "success");
						}
			...
			function approve_data(id)
			{
				if(confirm('Are you sure approve this data?'))
				{
					...
					$.ajax({
						url : "<?php echo site_url('approve/ajax_update_qco')?>/"+id+"/<?=$this->session->userdata('username')?>",
						type: "POST",
						dataType: "JSON",
						success: function(data)
						{
							...
							if(data.cekId == 'Success'){
								swal("Success", "Data Berhasil Approve!", "success");
							}

							if(data.cekId == 'Gagal'){
								swal("Gagal", "Data Sudah Approve oleh login "+data.upd_qco+"!", "error");
							}
